...
class TestLogin:
    ...
    def test_login_wrong_password(self, client, admin_user):
        r = client.post("/api/auth/login", json={"email": "admin@test.com", "password": "WrongPass1"})
        assert r.status_code == 401

    def test_login_nonexistent_email(self, client):
        r = client.post("/api/auth/login", json={"email": "noone@test.com", "password": "SomePass1"})
        assert r.status_code == 401
    ...
    def test_login_inactive_user(self, client, db):
        ...
        db.add(user)
        db.commit()
        r = client.post("/api/auth/login", json={"email": "inactive@test.com", "password": "Active1234"})
        assert r.status_code == 401
...
class TestRefresh:
    ...
    def test_refresh_invalid_token(self, client):
        r = client.post("/api/auth/refresh", json={"refresh_token": "bad.refresh.token"})
        assert r.status_code == 401
